                                    ...
                                    @foreach($portfolio as $item)
                                        <tr>
                                            ...
                                            <td style="display:flex;flex-wrap:wrap;width:190px;align-items: center;height: 168px;">
                                                <a href="{{route('portfolio.show', $item->id)}}" class="btn btn-success mr-1 mb-1 waves-effect waves-light">Просмотр</a>
                                                <a href="{{route('portfolio.edit', $item->id)}}" class="btn btn-warning mr-1 mb-1 waves-effect waves-light"><i class="feather icon-edit"></i> Изменить</a>
                                                <form action="{{route('portfolio.destroy',$item->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="btn btn-danger mr-1 mb-1 waves-effect waves-light"><i class="feather icon-trash-2"></i> Удалить</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
